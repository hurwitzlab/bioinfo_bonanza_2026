...
three major flavors of RNA-seq experiment (bulk, single-cell, and spatial),
the bioinformatics workflow that turns raw sequencing reads into a usable
count matrix, the statistics behind differential expression analysis, and how
those counts can be extended into gene co-expression networks.
...
| Aspect | Bulk RNA-seq | scRNA-seq | Spatial Transcriptomics |
|---|---|---|---|
| Definition | Gene expression across tissues and cell populations. | Sequencing of single cells to obtain their individual transcriptomes. | Simultaneous measurement of gene expression and the spatial location of cells. |
| Sample processing | Direct RNA extraction and amplification of tissue or cell populations. | Isolation, lysis, and amplification of individual cells. | Sectioning or fixation according to sample needs. |
| Experimental cost | Relatively low. | Higher. | Higher. |
| Data quality | Affected by RNA extraction and amplification quality; some batch effect. | Harder to obtain high-quality data — fluorescent labeling of individual cells and library prep both introduce noise. | Can be difficult, but provides high-precision spatial information. |
| Data analysis | Relatively simple; commonly uses standard differential expression methods. | Complex — requires analysis of individual cell characteristics. | Needs to combine molecular biology, imaging, and other fields for a full picture. |
| Best suited for | Studying overall gene expression and function across tissues or populations. | Studying individual cell phenotypes and developmental trajectories. | Studying the spatial structure of cell types and cellular interactions within tissue. |
...
## Choosing the Right Approach: Worked Examples

Picking among the three approaches comes down to matching the resolution of
the method to the resolution of the question you're actually asking. The
following scenarios walk through that reasoning.

**Chemotherapy resistance in a feline mammary tumor.** Suppose you're
studying why some cells in a tumor resist chemotherapy while others don't,
and you suspect that different tumor subpopulations are behaving differently.
Bulk RNA-seq would average signals across all the cells in the sample,
masking exactly the resistant subpopulation you're trying to find.
Single-cell RNA-seq is the right call here: it lets you identify distinct
cell clusters within the tumor and pinpoint which cells are driving
resistance, which is essential information for designing a targeted therapy.

**Liver response to canine distemper virus across a cohort.** Now suppose
you're comparing liver transcriptomes between infected and healthy dogs
across a cohort of 30 animals, looking for differentially expressed genes
between the groups. This is a classic bulk RNA-seq use case: you're comparing
overall gene expression across groups at scale, bulk sequencing is
cost-effective, it's statistically powerful for differential expression
testing, and single-cell resolution simply isn't needed to answer this
particular question.

**Equine neuroaxonal dystrophy and spatial neuroinflammation.** If the
question is which brain regions show abnormal gene expression, and whether
there's a spatial gradient of neuroinflammation from the core of a lesion out
to its periphery, spatial transcriptomics is the tool that fits. It maps gene
expression directly within tissue sections while preserving anatomical
context, so you can visualize the inflammation gradient and understand how
the lesion microenvironment is organized — information that both bulk and
single-cell approaches would lose entirely, since neither preserves the
tissue's physical layout.

**A poorly healing canine wound.** Imagine biopsying a wound and wanting to
know where fibroblasts are being activated, how the wound margin differs from
its center, and which signaling pathways are spatially coordinated. Again,
spatial transcriptomics is the answer, because it preserves tissue
architecture — showing gene expression *in situ* and revealing activation
gradients and cell-cell communication zones that neither bulk nor
single-cell sequencing can localize.

**Immune tolerance in bovine intestinal mucosa.** If you're trying to
understand how different immune cell populations — T cells, macrophages,
dendritic cells — coordinate their responses to commensal bacteria,
single-cell RNA-seq is the right resolution. It lets you dissect the immune
landscape cell by cell, identify rare cell populations, and determine how
each cell type individually responds. Bulk RNA-seq would mix all of those
signals together and mask the differences between populations.

**Evaluating a new avian influenza vaccine.** Finally, suppose you need to
compare whole-blood transcriptomes of vaccinated versus unvaccinated chickens
at three, seven, and fourteen days post-vaccination, across fifty birds. A
large cohort, multiple time points, and a group-level comparison is exactly
what bulk RNA-seq is designed for: it's cost-effective at this scale and
gives you the statistical power needed to draw conclusions across fifty
animals. Single-cell sequencing would be prohibitively expensive here, and
it isn't necessary to answer the question being asked.
...
Two caveats are worth flagging early. First, raw counts are not directly
comparable between samples — different samples can have different total
sequencing depth, so counts need to be normalized before they can be compared
meaningfully (more on this below). Second, and more fundamentally: a bad
experimental design cannot be fixed computationally. Replication,
randomization, and controlling for batch effects all have to happen *before*
sequencing — no amount of downstream statistical cleverness recovers an
experiment that wasn't designed properly in the first place.
...
divergent from the reference. A very high multimapping rate — reads that map
...
loci and being effectively unresolvable). Uneven coverage across a gene
...
by looking at individual genes in a genome browser like IGV. A
strand-specificity mismatch — where the alignment parameters don't match how
...
choosing alignment parameters. Finally, inconsistent mapping rates across
...
meaning the variance is greater than the mean. Most genes have low counts,
a handful of genes (housekeeping genes, ribosomal genes) have very high
counts, and counts are bounded at zero, since you can't have a negative
number of reads. That zero boundary forces any spread in the data to go
rightward.
